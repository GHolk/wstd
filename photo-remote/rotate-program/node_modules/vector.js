...
(function() {
  ...
  vector.dot = function(v) {
    return this.multiply(v).reduce((function(s, x) {
      return s + x;
    }), 0);
  };
  ...
  vector.multiplyNumber = function(n) {
    return this.map(function(x) {
      return n * x;
    });
  };

  vector.multiplyVector = function(v) {
    return this.map(function(x, i) {
      return x * v[i];
    });
  };

  vector.multiply = function(o) {
    if (typeof o === 'number') {
      return this.multiplyNumber(o);
    } else if (o instanceof Array) {
      return this.multiplyVector(o);
    }
  };

  vector.power = function(n) {
    var j, prod, ref, times;
    prod = vector.createFromNumber(1);
    for (times = j = 1, ref = n; 1 <= ref ? j <= ref : j >= ref; times = 1 <= ref ? ++j : --j) {
  ...
  vector.minus = function(v) {
    return this.multiply(-1).add(v).multiply(-1);
  };
