...
function solveLeastSquare(points) {
    ...
    */
    ...
    points.forEach((p) => {
        const rl = p.real
        const poly = [1, rl[X], rl[Y], rl[X]*rl[X], rl[Y]*rl[Y], rl[X]*rl[Y]]
        const zero6 = [0,0,0,0,0,0]
        eqn.push(poly.concat(zero6))
        eqn.push(zero6.concat(poly))
    ...
    const a = unkown.slice(0,6)
    const b = unkown.slice(6,12)
    ...
    return {
        ...
        realToPixel: function (real) {
            const order = [
                1, real[X], real[Y],
                real[X]*real[X], real[Y]*real[Y], real[X]*real[Y]
            ]
            const pixel = [0, 0]
            pixel[X] = this.coef[X]
                .map((c, i) => c * order[i])
